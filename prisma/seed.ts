...
const prisma = new PrismaClient({
	adapter: new PrismaPg({
		connectionString:
			...
			'postgresql://postgres:postgres@localhost:5432/hms',
	}),
});
...
async function main() {
	...
	const studentsRaw = [
		...
		{
			name: 'Amit Singh',
			email: 'amit@student.local',
			sn: 'S24003',
			prog: 'BCA',
		},
		{
			name: 'Sneha Kapur',
			email: 'sneha@student.local',
			sn: 'S24004',
			prog: 'B.Arch',
		},
		{
			name: 'Rohit Verma',
			email: 'rohit@student.local',
			sn: 'S24005',
			prog: 'BBA',
		},
		{
		...
		{
			name: 'Vikram Rathore',
			email: 'vikram@student.local',
			sn: 'S24007',
			prog: 'B.Tech CS',
		},
		{
			name: 'Ishaan Mehta',
			email: 'ishaan@student.local',
			sn: 'S24008',
			prog: 'MCA',
		},
		{
			name: 'Kavya Iyer',
			email: 'kavya@student.local',
			sn: 'S24009',
			prog: 'MBA',
		},
		{
		...
		const student = await prisma.student.create({
			data: {
				...
				admissionDate: new Date(
					ninetyDaysAgo.getTime() + i * 2 * 24 * 60 * 60 * 1000,
				),
			},
		});
		...
		data: {
			...
			roomId: (
				await prisma.room.findFirst({
					where: { building: 'Alpha', roomNumber: '101' },
				})
			)?.id,
	...
	const complaint = await prisma.complaintTicket.create({
		data: {
			...
			description:
				'WiFi is almost unusable in Beta block 2nd floor during peaks.',
			status: 'IN_PROGRESS',
			priority: 2,
	...
	await prisma.complaintLog.create({
		data: {
			...
			status: 'IN_PROGRESS',
			comment: 'Warden is looking into ISP settings',
			changedByUserId: warden.userId,
		},
	});
